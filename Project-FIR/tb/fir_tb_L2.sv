...
module fir_tb_L2;
    ...
    logic signed [DATA_W-1:0] data_in0, data_in1, data_out0, data_out1;
    ...
    int sample_idx = 0;
    initial begin
        valid_in = 0; data_in0 = '0; data_in1 = '0; rst_n = 0;
        #(CLK_PERIOD * 10); rst_n = 1; #(CLK_PERIOD * 2);
        
        for (int i = 0; i < NUM_SAMPLES/2; i++) begin
            @(posedge clk);
            valid_in  <= 1'b1;
            data_in0  <= stimulus_mem[2*i];
            data_in1  <= stimulus_mem[2*i+1];
        end
    ...
    always_ff @(posedge clk) begin
        if (valid_out) begin
            logic signed [DATA_W-1:0] exp0, exp1;
            ...
            if (data_out0 !== exp0 && data_out0 !== exp0+1 && data_out0 !== exp0-1)
                $display("ERR L2 [%0d]: Got %d, Exp %d", sample_idx, data_out0, exp0);
            if (data_out1 !== exp1 && data_out1 !== exp1+1 && data_out1 !== exp1-1)
                $display("ERR L2 [%0d]: Got %d, Exp %d", sample_idx+1, data_out1, exp1);
            sample_idx += 2;
        end
    end
    ...
    always @(posedge clk) begin
        ...
        if (valid_in) begin
            ...
            #(CLK_PERIOD/2.0) data_in_recon <= data_in1;
        end else begin
            data_in_recon <= '0;
        end
    end
    ...
    always @(posedge clk) begin
        ...
        if (valid_out) begin
            ...
            #(CLK_PERIOD/2.0) data_out_recon <= data_out1;
        end else begin
            data_out_recon <= '0;
        end
    end
